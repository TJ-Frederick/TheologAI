...
export function recommendedToolCallsForPrompt(
  ...
  switch (name) {
    ...
    case 'confession-study': {
      ...
      const localQuery = initialPrimarySourceLocalQuery(topic);
      return [{
...
2. **Search local evidence first** — Keep the user's full question for synthesis, but use one to three concise content terms in each local \`text\` field; preserve quoted wording and every requested catalog filter. Start with \`all_terms\`, using \`phrase\` only when exact wording is the research target. ${callText(calls[0])}. This initial \`searchDepth:"standard"\` plan is the authoritative local pass. Requested traditions are comparison interests, not creator filters or inferred metadata.
3. **Reformulate locally before broadening** — If an eligible catalog scope returns \`no_results\`, retry that scope once with a shorter or historically attested local expression, still using \`searchDepth:"standard"\` and the same work, creator, and year filters. Keep both query attempts in the research ledger. This bounded local retry is not an external-expansion trigger by itself.
4. **Conditionally broaden one proven gap** — Stop locally when the returned work-diverse evidence is useful. If the final local result proves \`catalog_miss\` or \`no_results\`, or a comparison/survey returns fewer than three distinct local \`documentId\` values, automatically rerun at most one affected query with \`searchDepth:"expanded"\`, \`expandedLimit:3\`, and an exact \`expansionBasis\`: \`{"reason":"catalog_miss"}\`, \`{"reason":"no_results"}\`, or \`{"reason":"insufficient_diversity","minimumDistinctWorks":3,"observedDistinctWorks":N}\`. Do not expand for metadata uncertainty, provider failure, or a merely subjective desire for more material.
5. **Handle uncertainty honestly** — If local coverage is useful but its adequacy is genuinely uncertain, say that broader or more detailed sources can be requested. A v8 \`expansionDecision\` records only whether the supplied basis was revalidated; it does not claim any later page was read.
6. **Read evidence before comparison** — Every snippet is discovery-only. Read at most five unique exact local resources; open a returned external URL independently before treating that page as evidence. Keep unreviewed external metadata separate from local evidence.
7. **Close the coverage ledger** — Copy searched/not-searched execution facts from the tool. Record read only after an exact resource or page opens successfully, and deferred only when you intentionally leave a selected lead unread with a reason.
8. **Compare cautiously** — Explain agreement, divergence, Scripture use, and historical context only from exact sections actually read. Missing hits are not historical silence. ${CCEL_DATE_CAPABILITY_NOTICE}`
          : descriptor.contractVersion === '7'
          ? `Cross-tradition doctrinal comparison on "${topic}".${hint}
...
2. **Run the standard local plan** — Keep the full user question for synthesis. Put one to three concise content terms in each query \`text\`; preserve quoted wording and every work, creator, and year filter. Start with \`all_terms\`, and use \`phrase\` only for wording that must be exact. ${callText(calls[0])}. Broad historical-perspective questions with useful local coverage stop here. Exact-work location uses relevance; surveys and creator comparisons preserve deterministic work diversity and separate creator scopes. ${localDateScope}
3. **Try one bounded local reformulation** — If an eligible catalog scope returns \`no_results\`, retry that affected scope once with a shorter or historically attested expression, still using \`searchDepth:"standard"\` and the same filters. Record both attempts. Do not treat the reformulation itself as permission to search an external provider.
4. **Automatically retry one proven targeted gap** — For a targeted author, work, period, or comparison scope, rerun at most one affected query with \`searchDepth:"expanded"\`, \`expandedLimit:3\`, and an \`expansionBasis\` only after the final standard result proves \`catalog_miss\` or \`no_results\`. For a survey/comparison diversity gap, count distinct returned local locator \`documentId\` values and use \`{"reason":"insufficient_diversity","minimumDistinctWorks":3,"observedDistinctWorks":N}\`. The other exact bases are \`{"reason":"catalog_miss"}\` and \`{"reason":"no_results"}\`. Copy every other query field unchanged. The server reruns local search and must revalidate that basis before any external provider can run.
5. **Do not manufacture a trigger** — Metadata-incomplete or unavailable local scope is uncertainty, not a coverage miss. In that case, or when useful local results leave genuine doubt, tell the user that broader or more detailed sources can be requested. Do not build or imitate a natural-language intent classifier.
6. **Interpret v8 routing and evidence truthfully** — Preserve \`expansionDecision\`, provider order, statuses, notices, result windows, and coverage facts. ${externalDateSafeguard} ${externalDateCallInstruction} External snippets remain unreviewed discovery leads and are never evidence until their direct pages are opened and verified.
7. **Read selected exact evidence** — Follow at most ${args?.maxSections?.trim() || '3'} unique selected locators. Use MCP \`resources/read\` for local resources. Do not quote, compare, or synthesize from snippets alone.
8. **Close the host ledger and synthesize** — Searched/not-searched come only from the tool; read and deferred are host-observed later. Distinguish exact text from interpretation, disclose unread external leads and unsearched scopes, and never treat a bounded miss as historical silence.`
          : descriptor.contractVersion === '7'
          ? `Research primary-source evidence about "${topic}"${work ? ` within the requested work "${work}"` : ''}${authors.length ? ` for the separately scoped creators ${authors.map(value => `"${value}"`).join(', ')}` : ''}.
...
2. **Run bounded discovery** — Keep the user's full question for synthesis. Put one to three concise content terms in each query \`text\`, preserving quoted wording and every work, creator, and year filter; start with \`all_terms\` and reserve \`phrase\` for exact wording. ${callText(calls[0])}. This workflow is local-only: do not claim it searched CCEL, the web, an exhaustive catalog, or works outside the server's collection. Topic and creator surveys use deterministic work diversity; exact within-work location uses relevance.
3. **Try one bounded local reformulation** — If an eligible catalog scope returns \`no_results\`, retry that affected scope once with a shorter or historically attested expression and the same filters. Record both attempts. A second miss remains a bounded collection result, not historical silence.
4. **Use the v6 structured result** — Preserve each separate creator query, \`normalizedSelection\`, provider status, \`resultWindow\`, catalog \`scope\` status/count/work list/truncation, rank, notices, creator roles, document type/date, canonical \`sectionKey\`/\`sourceOrdinal\` locator, edition readiness, and \`evidencePolicy\`. \`additional_match_observed\` means only that at least one more match was seen beyond the returned window, not that the corpus was exhaustively counted. A \`catalog_miss\` means the hosted catalog did not match the restriction; \`no_results\` means eligible hosted works were searched but no text hit matched. Treat every snippet as \`discovery_only\`.
5. **Read selected evidence before quotation or comparison** — Follow at most ${args?.maxSections?.trim() || '3'} unique canonical \`resource_link\` blocks using MCP \`resources/read\`. Deduplicate locators and confirm that every returned URI matches the selected locator. Do not quote, compare creators/works, or draw substantive conclusions from snippets alone.
6. **Report provenance honestly** — Edition provenance is incomplete and rights status is not established by the catalog resource. Do not invent an author, edition, transcription source, publication date, or rights status. Work-level type/date metadata is not edition metadata.
7. **Close the coverage ledger** — Report searched from provider states; count a section as read only after its exact MCP resource returned successfully; list intentionally deferred selected sections with reasons; and list not-searched scope only when the server reported non-execution. Do not infer later reads or deferrals from search output.
8. **Synthesize with limits** — Distinguish what the selected sections say from your interpretation. Name searches that returned no results or unsupported filters; do not treat missing hits as historical silence.
