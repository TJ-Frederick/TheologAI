...
type DeepFrozen<T> = T extends readonly (infer Value)[]
  ? readonly DeepFrozen<Value>[]
  : T extends object
    ? { readonly [Key in keyof T]: DeepFrozen<T[Key]> }
    : T;

/** Freezes static commitment graphs once at module initialization, including nested arrays and evidence objects. */
...
const A1_PACKAGE_IDENTITY_ANCHOR = deepFreeze({ workId: 'thomas-aquinas-summa-theologiae', editionId: 'aquinas-summa-english-dominican-gutenberg-electronic', collectionId: 'aquinas-summa-pg-v1', contentFormat: 'plain_text', availability: 'local_only_inactive' } as const);
export const AQUINAS_A1_PACKAGE_IDENTITY = A1_PACKAGE_IDENTITY_ANCHOR;
const A1_SOURCE_COMMITMENTS_ANCHOR = deepFreeze([
...
export const AQUINAS_A1_SOURCES = A1_SOURCE_COMMITMENTS_ANCHOR;
...
const A1_SOURCE_TYPED_RANGE_COMMITMENTS_ANCHOR = deepFreeze([
...
const A1_DISCREPANCY_INVENTORY_ANCHOR = deepFreeze(
  (JSON.parse(AQUINAS_A1_DISCREPANCY_PROJECTION_JSON) as A1DiscrepancyLedgerEntry[]).map((entry, index) => ({
    ref: `a1-ledger-${String(index + 1).padStart(3, '0')}`,
    canonicalEntrySha256: legacyA1DiscrepancyEntrySha256(entry),
    ...entry,
  })),
);
export const AQUINAS_A1_DISCREPANCY_INVENTORY = A1_DISCREPANCY_INVENTORY_ANCHOR;
...
export function validateImmutableA1EvidenceDescriptor(): void {
  if (A1_SOURCE_COMMITMENTS_ANCHOR.length !== 4) fail('A1.sources', 'must enumerate exactly four HTML artifacts');
  if (A1_TOPOLOGY_COMMITMENT_ANCHOR.questionCount !== 512 || A1_TOPOLOGY_COMMITMENT_ANCHOR.preambleCount !== 512 || A1_TOPOLOGY_COMMITMENT_ANCHOR.articleCount !== 2_669) fail('A1.topology', 'must bind 512 questions, 512 preambles, and 2669 articles');
  if (A1_DISCREPANCY_INVENTORY_ANCHOR.length !== 46) fail('A1.discrepancies', 'must bind the ordered 46-entry discrepancy inventory');
  const prologues = A1_TOPOLOGY_COMMITMENT_ANCHOR.partPrologues.map(value => `${value.partKey}:${value.count}`).join(',');
  if (prologues !== 'prima:1,prima-secundae:1,secunda-secundae:0,tertia:1') fail('A1.prologues', 'must include Prima, I-II, and III only');
  if (A1_TOPOLOGY_COMMITMENT_ANCHOR.authorialChildRangeCount !== 3_184 || A1_SOURCE_TYPED_RANGE_COMMITMENTS_ANCHOR.reduce((count, source) => count + source.typedRangeCount, 0) !== 3_188) fail('A1.typedRanges', 'must separately bind 3,184 authorial child ranges and the four native source typed-range projections');
  if (A1_TOPOLOGY_COMMITMENT_ANCHOR.partQuestionPreambles.map(value => value.count).join(',') !== '119,114,189,90' || A1_TOPOLOGY_COMMITMENT_ANCHOR.partArticles.map(value => value.count).join(',') !== '584,619,917,549') fail('A1.topology', 'must retain exact per-part preamble and article counts');
  if (A1_RIGHTS_AND_COVERAGE_ANCHOR.authorialCoverageThrough !== 'tertia.q090' || A1_RIGHTS_AND_COVERAGE_ANCHOR.supplement !== 'excluded') fail('A1.coverage', 'must stop authorial delivery at III.90 and exclude the Supplement');
}
...
export function validateA1SourceTypedRangeProjection(input: unknown): void {
  ...
  equalCanonical(values, A1_SOURCE_TYPED_RANGE_COMMITMENTS_ANCHOR, 'A1.sourceTypedRanges', 'must exactly bind all four native topology-lock typed-range commitments');
}
...
export function validateA1DiscrepancyProjection(input: unknown): void {
  ...
  equalCanonical(values, A1_DISCREPANCY_INVENTORY_ANCHOR, 'A1.discrepancies', 'must exactly bind every ordered A1 discrepancy projection field');
}
...
function validateExactIdentity(input: unknown, path: string): typeof AQUINAS_A1_PACKAGE_IDENTITY {
  ...
  for (const key of Object.keys(A1_PACKAGE_IDENTITY_ANCHOR) as (keyof typeof A1_PACKAGE_IDENTITY_ANCHOR)[]) literalAt(root[key], `${path}.${key}`, A1_PACKAGE_IDENTITY_ANCHOR[key]);
  return { ...A1_PACKAGE_IDENTITY_ANCHOR };
}
...
function validateRightsAndCoverage(input: unknown, path: string): typeof AQUINAS_A1_RIGHTS_AND_COVERAGE {
  ...
  equalCanonical(root, A1_RIGHTS_AND_COVERAGE_ANCHOR, path, 'must exactly bind the immutable US-only rights, lineage, limitations, and coverage descriptor');
  return structuredClone(A1_RIGHTS_AND_COVERAGE_ANCHOR);
}
...
function validateExclusion(input: unknown, path: string, artifacts: ReadonlyMap<string, SourceArtifactEvidence>): TransientExclusion {
  ...
  return { exclusionId: safeIdAt(root.exclusionId, `${path}.exclusionId`), kind: enumAt(root.kind, `${path}.kind`, A1_RIGHTS_AND_COVERAGE_ANCHOR.exclusionKinds), artifactId, span, html };
}
...
function assertA1AttestedDraft(draft: TransientSectionedEditionCollectionDraft): void {
  ...
  literalAt(draft.typedRangeCount, '$.typedRangeCount', A1_TOPOLOGY_COMMITMENT_ANCHOR.authorialChildRangeCount);
  ...
  equalCanonical(
    ...
    A1_SOURCE_COMMITMENTS_ANCHOR.map(({ ebookId, ...source }) => ({ artifactId: `pg-${ebookId}`, ...source })),
    '$.sourceArtifacts',
  ...
  literalAt(domainHash('sectioned-edition-collection-package.question-vector.v1', canonicalSectionedEditionCollectionPackageBytes(questions)), '$.questions', A1_TOPOLOGY_COMMITMENT_ANCHOR.questionVectorSha256);
  const articleKeys = draft.questions.flatMap(question => question.articles.map(article => article.articleKey!));
  literalAt(articleKeys.length, '$.questions.articles', A1_TOPOLOGY_COMMITMENT_ANCHOR.articleCount);
  literalAt(domainHash('sectioned-edition-collection-package.article-vector.v1', canonicalSectionedEditionCollectionPackageBytes(articleKeys)), '$.questions.articles', A1_TOPOLOGY_COMMITMENT_ANCHOR.articleVectorSha256);
  equalCanonical(draft.discrepancies, A1_DISCREPANCY_INVENTORY_ANCHOR, '$.discrepancies', 'must bind every exact ordered A1 discrepancy code, basis, observed, and resolved projection field');
  const expectedInventory = A1_DISCREPANCY_INVENTORY_ANCHOR.map(value => [value.ref, value.canonicalEntrySha256, value.questionKey, value.articleKey]);
  ...
  for (const question of draft.questions) {
    const expectedRefs = A1_DISCREPANCY_INVENTORY_ANCHOR.filter(value => value.questionKey === question.questionKey).map(value => value.ref);
...
function assertA1NativeSourceTypedRangeProjection(draft: TransientSectionedEditionCollectionDraft): void {
  for (const expected of A1_SOURCE_TYPED_RANGE_COMMITMENTS_ANCHOR) {
