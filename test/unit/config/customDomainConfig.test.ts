...
describe('custom-domain infrastructure contract', () => {
  ...
  it('publishes canonical deployment URLs without weakening preview authorization', async () => {
    ...
    expect(releaseContext).toContain("if [ '${{ github.event_name }}' = 'workflow_dispatch' ]; then");
    expect(releaseContext).toContain('before="$(git rev-parse HEAD^1)"');
    expect(releaseContext).toContain("before='${{ github.event.before }}'");
    expect(releaseContext).toContain('custom_domain_required=true');
    expect(releaseContext).toContain('custom_domain_required=false');
