...
describe('published project contract', () => {
  ...
  it('keeps current preview evidence distinct from historical preview releases', async () => {
    ...
    const currentPreviewD1 = 'theologai-preview-20260725-t10-a';
    const currentPreviewD1Id = 'fbd1a492-fbc2-4061-a431-181a9632d4de';
    const preparedCandidateD1 = 'theologai-preview-20260727-normal-a';
    const preparedCandidateD1Id = '776944d4-60d1-457f-b13e-b4e7898971ca';
    ...
    for (const document of [reconciliation, operations]) {
      ...
      expect(document).toContain(currentPreviewD1);
      expect(document).toContain(currentPreviewD1Id);
    }
    expect(reconciliation).toContain('historical read-only observation');
    expect(reconciliation).toContain('currently live preview Worker still uses the older Transform-10-era D1');
    expect(reconciliation).toContain('separately prepared but not-yet-deployed normal-build candidate');
    expect(reconciliation).toContain('This candidate is unbound from the active Worker');
    expect(operations).toContain('historical, read-only Cloudflare snapshot');
    expect(operations).toContain('currently live preview Worker still uses the older Transform-10-era D1');
    expect(operations).toContain('prepared but not-yet-deployed normal-build D1');
    expect(operations).toContain('No preview Worker deployment or binding change is\nrecorded for this candidate');
    expect(operations).toContain('new\nnormal-build candidate is prepared but unbound, excludes the inactive Aquinas\nhierarchy rows');
    expect(operations).toContain('live older Transform-10-era preview D1 remains active until\ncutover; the prepared but unbound normal-build candidate excludes inactive\nAquinas hierarchy rows');
    expect(operations).toContain('Transform-10 Aquinas hierarchy remains\nlocal-only and inactive; the prepared normal-build candidate excludes its\nhierarchy and lineage');
    expect(operations).not.toContain('The prepared but unbound preview\ncandidate retains that inactive authority data');
    expect(operations).not.toContain('its prepared preview candidate');
    expect(operations).not.toContain('Transform 10 is an unpublished, inactive hierarchy candidate;\nits prepared but unbound preview D1');
    ...
    expect(reconciliation).toContain('it makes\nno production claim');
