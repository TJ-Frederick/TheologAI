...
This is a local-only capacity measurement of the checked-in inactive Project
Gutenberg Aquinas package after Transform 10 materializes its generic
edition-scoped hierarchy. It does not authorize runtime wiring, binding
changes, deployment, remote D1 work, Cloudflare operations, or a source-pack
release.
...
targets. It builds and verifies a fresh Transform 10 database only below the
operating system temporary directory, measures it before `VACUUM`, reports the
manifest-derived corpus identity, then removes every temporary file. Tests may
inject a disposable fixture baseline or builder; the CLI cannot.
...
## Transform 10 materialization

Transform 10 keeps the 3,184 individual authority bodies in generic,
...
The capacity gate is the directly measured fresh Transform 10 database **before
`VACUUM`**, after materialization and `ANALYZE`. The report exposes this as
`actualTransform10.preVacuumFullCopy`, and `capacityGate.finalBytes` is exactly that
file size. It prints the complete result and exits nonzero when Transform 10
exceeds 350 MiB.
...
| Transform | Direct `preVacuum` | Post-`VACUUM` diagnostic | 350 MiB gate |
|---|---:|---:|---|
| 10: 3,184 bodies and 4/512/2,669 implicit-root navigation | 339,603,456 | 325,775,360 | within by 27,398,144 bytes |
...
Transform 10 has a reviewed local materializer, manifest allowlist, fresh-build
verification, and D1-seed export/import verification. It remains inactive:
